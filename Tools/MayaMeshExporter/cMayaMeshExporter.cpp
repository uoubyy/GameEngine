...
namespace
{
		...
		const std::vector<sMaterialInfo>& i_materialInfo )
	{
		...
		if ( fout.is_open() )
		{
			...
			fout << "-- " << i_vertexArray.size() << " vertices\n";
			fout << "-- " << i_indexArray.size() / 3 << " triangles\n";
			fout << "return" "\n"
				"{" "\n";
			{
				...
				fout << "\tvertices =\n";
				fout << "\t{\n";
				for ( size_t idx = 0; idx < i_vertexArray.size(); ++idx )
				{
					const auto& vertex = i_vertexArray[idx];
					fout << "\t\t{ ";
				...
				fout << "\telements =\n";
				fout << "\t{\n";
				for ( size_t idx = 0; idx < i_indexArray.size(); idx += 3 )
				{
					fout << "\t\t{ ";
					fout << i_indexArray[idx] << ", " << i_indexArray[idx + 2] << ", " << i_indexArray[idx + 1];
					fout << " },\n";
				}
				fout << "\t},\n";
			}
